...
describe Validations::SufficientBetAmountValidationService do
  let!(:account) { Fabricate(:account) }

  context 'game with valid bet amount' do
    let(:game) { Fabricate(:game_in_validation, account: account) }
  ...
  context 'game with invalid bet amount' do
    let(:game) { Fabricate(:game_max_bet_amount_invalid, account: account) }
